...
| **Live layer** | The layer currently selected in the GUI. It scopes *everything* downstream — ordering, hover surface, normals, precompute, cache file, collision obstacle, playback — and drives scene visibility. **Functional, not cosmetic** — `Surface_RX_Offset` is sealed inside the `Surface_TX_Base` shell, so RX is *invisible* with TX_Base shown. 6.3 applies **strict** isolation (only the selected layer visible); the eventual 6.6 rule is the S1.32 physical stack (RX shows itself + below; TX shows the whole object incl. RX beneath). | `apply_live_layer_visibility()`; see `settled.md` S1.32, S1.35 |
...
| **Live-layer isolation** | The GUI layer selector shows **one layer at a time** — picking RX enables only RX's surface/overlay/travel and hides all TX geometry (needed because RX is sealed inside the TX shell). Currently *strict* isolation for verification; the eventual 6.6 behaviour is the S1.32 physical stack (TX shows RX beneath). Distinct from the sample-geodesic isolation, with which it composes. | `apply_live_layer_visibility()`; see `settled.md` S1.32, S1.35 |
