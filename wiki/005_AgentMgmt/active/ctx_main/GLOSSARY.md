...
| **Orientation frame (per-waypoint)** | The target TCP orientation at one printed feed point, holding the nozzle perpendicular to the curved surface — a 3×3 `R_target` whose **Z axis is the outward surface normal** (nozzle approaches along −Z) and whose in-plane X/Y are pinned to a **fixed world reference**, *not* the path tangent (the nozzle is rotationally symmetric, so the spin DOF is free and is fixed to keep the arm from spinning as the path meanders). Per layer, stored as `curved_orient_frames` — a list of `(pos_world, R_target)` in print order, the array 6.5 feeds to IK. Drawn as a downsampled XYZ triad overlay (`Curved Orient Frames <name>`). Supersedes the flat-plate single constant `R_target` (S1.12). | `build_orientation_frames()`; see `settled.md` **S1.36** |
