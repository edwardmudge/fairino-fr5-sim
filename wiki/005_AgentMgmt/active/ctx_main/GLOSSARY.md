...
| **Live layer** | The layer currently selected in the GUI. It scopes *everything* downstream — ordering, hover surface, normals, precompute, cache file, collision obstacle, playback — and drives scene visibility: RX shows itself and everything below it (`Surface RX Offset`, `Surface Bot`); TX shows the whole object. That visibility rule is **functional, not cosmetic** — `Surface_RX_Offset` is sealed inside the `Surface_TX_Base` shell, so RX is *invisible* with TX_Base shown. | `_isolate_geodesic_layer()`/`_restore_geodesic_isolation()`; see `settled.md` S1.31, S1.32 |
