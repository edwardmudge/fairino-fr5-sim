...
| Orientation search / re-shaped rejection (Stage 7.4) | not started | **Planned only — nothing built.** A waypoint stops being judged by one commanded frame: the tool axis need only be perpendicular to the surface **within 20°** (supervisor), and the free roll DOF is **searched** (60 slots x 6°) rather than pinned by S1.36's `argmin \|a·z\|` — ~480 candidates per waypoint instead of 8. Adopts the nine candidate filters from `examples/curved_surface_printing/IK_BRANCH_REJECTION_GUIDE.md` (an **external** project's working implementation), including this project's **first mesh-vs-mesh collision** (surface 2.0mm, self 5.0mm), and replaces S1.40's infinite plate plane with a finite footprint (20mm margin) + slab (3.0mm). Selection becomes a Dijkstra search over a `(waypoint, candidate)` DAG, reusing S1.31's `heapq` implementation. Edge joint-step limit is **30°, not the reference's 35°** (the exchange spec rejects >30°). `PRECOMPUTE_CACHE_VERSION` **6→7**, shared with 7.5's cache-gap fix. ⚠ Net effect is **stricter**, not looser — only the *commanded pose* relaxes. ⚠ Whether this restores curved reachability is an **unmeasured prediction** — see `settled.md` **S1.46** |
| Job export / GUI (Stage 7.5–7.6) | not started | Planned only — see `tutorials/Stage7_README.md` and the inbox note (whose §7.4/§7.5 are the pre-renumber numbers) |
...
See `wiki/002_Architecture/settled.md` (S1.1–S1.46).
