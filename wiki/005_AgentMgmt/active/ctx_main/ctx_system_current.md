...
| Curved-surface print ordering (Stage 6.3) | not started, **unblocked** | Cost matrices from 6.2 exist; nothing consumes them yet. 6.3's blocking open question — which layer prints first — was answered 2026-07-20: **RX first**, then TX, with a manual silicone fill between the two passes (`settled.md` S1.32). The pass order, the per-pass collision obstacle and the per-layer visibility rule are all now specified, so this can start |
